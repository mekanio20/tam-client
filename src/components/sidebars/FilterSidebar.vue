    ...
    <div class="min-h-screen">
        ...
        <Transition enter-active-class="transition-transform duration-300 ease-out" enter-from-class="translate-x-full"
            ...
            <div v-if="showSidebar"
                class="fixed right-0 top-0 h-full w-full max-w-[360px] bg-[#F6F7F9] z-50">
                <!-- Header -->
                <div
                ...
                <div class="px-8 bg-white p-4 rounded-2xl">
                    ...
                    <div class="space-y-3 max-h-[300px] overflow-y-auto">
                        <label v-for="option in brands" :key="option.id"
                            class="flex items-center cursor-pointer group">
                            <input type="radio" :value="option.id" v-model="selectedBrend" class="sr-only">
                            <div :class="[
                'w-5 h-5 rounded-full border-2 mr-3 transition-all duration-200 flex items-center justify-center',
                selectedBrend === option.id
                    ? 'border-[#FEB918] bg-[#FEB918]'
                    : 'border-gray-300 group-hover:border-gray-400'
            ]">
                                <div v-if="selectedBrend === option.id" class="w-2 h-2 rounded-full bg-white" />
                            </div>
                ...
                <!-- Footer Buttons -->
                <div
                    class="border-t bg-white absolute bottom-2 left-0 right-0 rounded-2xl border-gray-100 p-6 flex gap-3">
